    ...
    if (item.substr(len - 6) === ActionSuffix) {
      actions[item] = function actionCreator() {
        ...
        if (typeof model[item] === 'function') {
          ...
          action = (_model$item = model[item]).apply.apply(_model$item, [this].concat(args));
...
var defaultMiddleware = [_reduxPromise2.default];
...
function moox(models) {
  ...
  var MOOX = {
    getStore: function getStore() {
      ...
      store = finalCreateStore((0, _redux.combineReducers)(reducers));
      return store;
    }
  };
